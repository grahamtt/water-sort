...
class GameState {
  ...
  bool get isSolved {
    // A puzzle is solved when all non-empty containers contain only one color
  ...
  List<Container> _reconstructContainersUpToMove(int targetMoveIndex) {
    ...
    final reconstructedContainers = initialContainers.map((c) => c.copyWith()).toList();
    ...
    for (int i = 0; i <= targetMoveIndex && i < moveHistory.length; i++) {
      ...
      sourceContainer.removeSpecificVolume(move.liquidMoved.color, move.liquidMoved.volume);
